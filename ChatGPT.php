...
class ChatGPT {
    ...
    protected $model = "gpt-4-0125-preview";
    protected $chat_id = null;
    //protected $api_key = "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";
    protected $api_key = "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";
    ...
    public function force_function_call($function_name,  $arguments = null ) {
    ...
    protected function get_functions() {
        ...
        foreach ($this->functions as $function) {
            ...
            $functions[] = [
                ...
                "parameters" => [
                    "type" => "object",
                    "properties" => $properties, // Ensure properties are properly set
                    "required" => $required,
                ],
            ];
        }
    ...
    public function add_function($function) {
        if (is_callable($function)) {
            $parsed_function = $this->parse_function($function);
            if ($parsed_function !== false) {
                $this->functions[] = $parsed_function;
            } else {
                // Handle error if the function cannot be parsed
                throw new \Exception("Failed to parse function.");
            }
        } else {
            // Handle error if the provided function is not callable
    ...
    protected function parse_function( callable $function ) {
        ...
        foreach( $params as $param ) {
            ...
            $type = $parameterType ? $parameterType->getName() : ($types[$index] ?? "");  
